...
def reload_recordings_list(node_path):
    ...
    byplay_access_token = node.parm(u"byplay_access_token").evalAsString()
...
def load_recording_for_ui(node_path):
    ...
    if not _validate_access_token(byplay_access_token):
        return
    byplay_recordings_dir = node.parm(u"byplay_recordings_dir").evalAsString()
    recording_id = node.parm(u"byplay_recording_id").evalAsString()

    if len(byplay_recordings_dir) < 3 or len(recording_id) < 18:
        hou.ui.displayMessage(u"Please fill in access token, recordings dir and select a recording", severity=hou.severityType.Error)
        return
...
def _validate_access_token(token):
    hou = get_hou()
    if len(token) < 5:
        hou.ui.displayMessage(u"Please fill in Byplay access token", severity=hou.severityType.Error)
        return False
